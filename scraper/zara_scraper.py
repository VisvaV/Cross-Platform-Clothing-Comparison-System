...
PLATFORM = "Zara"
BASE_URL = "https://www.zara.com/in/en/search?searchTerm={category}"


async def scrape_products(category: str, limit: int = 50) -> list[dict]:
    """Scrape Zara products for a given category.
    ...
    try:
        ...
        async with AsyncCamoufox(
            ...
            cards = await page.query_selector_all(
                "li.product, li[class*='product'], article[class*='product']"
            )
            ...
            for card in cards:
                ...
                try:
                    ...
                    logger.debug("Error parsing Zara card: %s", e)

            await random_delay()
            await browser.close()
    except Exception as e:
        logger.warning("Zara scraper failed for category '%s': %s", category, e)
        return []

    logger.info("Zara: scraped %d products for '%s'", len(results), category)
    return results
...
if __name__ == "__main__":
    import asyncio
    import sys
    cat = sys.argv[1] if len(sys.argv) > 1 else "dresses"
