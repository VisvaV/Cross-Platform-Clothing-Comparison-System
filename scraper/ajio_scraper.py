"""Ajio product scraper using Playwright."""
    ...
    try:
        async with async_playwright() as pw:
            ...
            cards = await page.query_selector_all(
                "div.item, div[class*='item '], li[class*='item']"
            )
            ...
            for card in cards:
                ...
                try:
                    brand = first_text(card, [
                        "strong.brand", "[class*='brand']",
                        "strong", "h3",
                    ])
                    title = first_text(card, [
                        "div.nameCls", "[class*='nameCls']",
                        "[class*='name']", "p",
                    ])
                    price_raw = first_text(card, [
                        "span.price strong", "[class*='price'] strong",
                    ...
                    logger.debug("Error parsing Ajio card: %s", e)

            await random_delay()
            await browser.close()
    except Exception as e:
        logger.warning("Ajio scraper failed for category '%s': %s", category, e)
        return []

    logger.info("Ajio: scraped %d products for '%s'", len(results), category)
    return results
