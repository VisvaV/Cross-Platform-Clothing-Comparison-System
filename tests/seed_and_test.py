...
TEXT_ID_MAP_PATH = "data/test_text_id_map.pkl"   # NEW
...
assert embeddings.shape == (len(MOCK_PRODUCTS), 2816)


# ── STEP 4: TF-IDF text embeddings + text_id_map ─────────────────────────────
...
# ── NEW: save text_id_map ─────────────────────────────────────────────────────
with open(TEXT_ID_MAP_PATH, "wb") as f:
    pickle.dump(ids, f)
logger.info("text_id_map saved: %d entries", len(ids))
...
searcher = SimilaritySearch(
    ...
    text_id_map_path=TEXT_ID_MAP_PATH,   # NEW
)
...
assert text_results[0]["similarity_score"] > 0

# ── TEXT SEARCH — multi-word colour query ─────────────────────────────────────
logger.info("--- TEXT SEARCH: 'green maxi dress' ---")
green_results = searcher.search_by_text("green maxi dress", k=5)
assert len(green_results) > 0, "Green dress text search returned no results"
print("\n[TEXT SEARCH] Query: 'green maxi dress'")
for i, r in enumerate(green_results, 1):
    print(f"  {i}. [{r['platform']}] {r['product_title']} — ₹{r['price']} (score: {r['similarity_score']:.4f})")
# Top result should be the green floral maxi dress
assert "green" in green_results[0]["product_title"].lower() or green_results[0]["similarity_score"] > 0

# ── IMAGE SEARCH ──────────────────────────────────────────────────────────────
logger.info("--- IMAGE SEARCH: red-ish image ---")
query_img = make_solid_image((210, 55, 55))
...
for i, r in enumerate(image_results, 1):
    print(f"  {i}. [{r['platform']}] {r['product_title']} — ₹{r['price']} (score: {r['similarity_score']:.4f})")
assert image_results[0]["similarity_score"] > 0

# ── HYBRID SEARCH ─────────────────────────────────────────────────────────────
logger.info("--- HYBRID SEARCH: blue image + 'blue denim jacket' ---")
query_img2 = make_solid_image((55, 90, 200))
with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp:
...
assert hybrid_results[0].get("hybrid_score", 0) > 0

# ── RANKING ───────────────────────────────────────────────────────────────────
from ranking.ranking_engine import rank_by_price, rank_by_score

price_ranked = rank_by_price(text_results)
# Products with None price go to the end; known prices should be ascending
known_prices = [r["price"] for r in price_ranked if r.get("price") is not None]
assert known_prices == sorted(known_prices), "rank_by_price not sorted ascending"
print("\n[RANK BY PRICE]")
